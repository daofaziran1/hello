...
void build(int l, int r, int ceng) {
    ...
    int mid = (l + r) / 2,
        isame =
            mid - l + 1;  // isame保存有多少和sorted[mid]一样大的数进入左孩子
    for (int i = l; i <= r; i++)
...
int look(int ceng, int sl, int sr, int l, int r, int k) {
    ...
    if (tolef >= k) {
        return look(ceng + 1, sl, (sl + sr) / 2, sl + ly, sl + num[ceng][r] - 1,
                    k);
    } else {
        // l-sl 表示l前面有多少数，再减ly 表示这些数中去右子树的有多少个
        int lr = (sl + sr) / 2 + 1 + (l - sl - ly);  // l-r 去右边的开头位置
        // r-l+1
        // 表示l到r有多少数，减去去左边的，剩下是去右边的，去右边1个，下标就是lr，所以减1
        return look(ceng + 1, (sl + sr) / 2 + 1, sr, lr,
                    lr + r - l + 1 - tolef - 1, k - tolef);
    }
}
